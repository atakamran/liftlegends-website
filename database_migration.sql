...
-- Gyms policies (public read, admin write)
CREATE POLICY "Anyone can view active gyms" ON gyms FOR SELECT USING (is_active = true);
CREATE POLICY "Admins can manage gyms" ON gyms FOR ALL USING (
    EXISTS (
...
-- Gym memberships policies (public read, admin write)
CREATE POLICY "Anyone can view active gym memberships" ON gym_memberships FOR SELECT USING (is_active = true);
CREATE POLICY "Admins can manage gym memberships" ON gym_memberships FOR ALL USING (
    EXISTS (
        SELECT 1 FROM profiles 
        WHERE profiles.user_id = auth.uid() 
        AND profiles.is_admin = true
...
-- Coaches policies (public read, coach/admin write)
CREATE POLICY "Anyone can view active coaches" ON coaches FOR SELECT USING (is_active = true);
CREATE POLICY "Coaches can manage their own profile" ON coaches FOR ALL USING (user_id = auth.uid() AND user_id IS NOT NULL);
CREATE POLICY "Admins can manage all coaches" ON coaches FOR ALL USING (
    EXISTS (
        SELECT 1 FROM profiles 
        WHERE profiles.user_id = auth.uid() 
        AND profiles.is_admin = true
...
-- Coach programs policies (public read, coach/admin write)
CREATE POLICY "Anyone can view active coach programs" ON coach_programs FOR SELECT USING (is_active = true);
CREATE POLICY "Coaches can manage their own programs" ON coach_programs FOR ALL USING (
    EXISTS (
        SELECT 1 FROM coaches 
        WHERE coaches.id = coach_programs.coach_id 
        AND coaches.user_id = auth.uid()
    )
);
CREATE POLICY "Admins can manage all coach programs" ON coach_programs FOR ALL USING (
    EXISTS (
        SELECT 1 FROM profiles 
        WHERE profiles.user_id = auth.uid() 
        AND profiles.is_admin = true
...
-- Athlete programs policies (athlete and coach can access, admin can manage all)
CREATE POLICY "Athletes can view their own programs" ON athlete_programs FOR SELECT USING (athlete_id = auth.uid());
CREATE POLICY "Coaches can manage programs for their athletes" ON athlete_programs FOR ALL USING (
    EXISTS (
        SELECT 1 FROM coaches 
        WHERE coaches.id = athlete_programs.coach_id 
        AND coaches.user_id = auth.uid()
    )
);
CREATE POLICY "Admins can manage all athlete programs" ON athlete_programs FOR ALL USING (
    EXISTS (
        SELECT 1 FROM profiles 
        WHERE profiles.user_id = auth.uid() 
        AND profiles.is_admin = true
...
-- Order items policies (users can access their own order items, admins can access all)
...
-- Sample coaches (without user_id for now - will be added when users register as coaches)
