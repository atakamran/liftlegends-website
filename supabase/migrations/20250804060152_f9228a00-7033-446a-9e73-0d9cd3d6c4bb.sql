-- Add missing user_id column to coaches table
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Create gyms table if it doesn't exist
...
-- Add other missing columns to coaches
...
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS specialties TEXT[];
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS experience_years INTEGER;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS certifications TEXT[];
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS profile_image TEXT;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS gallery TEXT[];
...
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS instagram TEXT;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS telegram TEXT;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS rating DECIMAL(3,2) DEFAULT 0.00;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS total_reviews INTEGER DEFAULT 0;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS is_verified BOOLEAN DEFAULT false;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS gym_id UUID;
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT now();
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT now();

-- Add foreign key constraint for gym_id after gyms table exists
ALTER TABLE public.coaches ADD CONSTRAINT fk_coaches_gym_id FOREIGN KEY (gym_id) REFERENCES public.gyms(id);
...
DROP POLICY IF EXISTS "Admins can manage gyms" ON public.gyms;
...
-- Insert sample gyms
INSERT INTO public.gyms (name, description, address, phone, email, facilities, working_hours, pricing, capacity) 
SELECT * FROM (VALUES
  ...
  ('باشگاه فیت لایف', 'باشگاه بانوان با فضای مدرن و امکانات کامل', 'تهران، خیابان شریعتی، پلاک ۷۸', '02144556677', 'info@fitlife.ir',
   ...
   120)
) AS v(name, description, address, phone, email, facilities, working_hours, pricing, capacity)
WHERE NOT EXISTS (SELECT 1 FROM public.gyms WHERE name = v.name);

-- Insert additional sample coaches
INSERT INTO public.coaches (name, bio, specialties, experience_years, certifications, phone, email, instagram, telegram, rating, total_reviews, is_verified, is_active) 
SELECT * FROM (VALUES
...
) AS v(name, bio, specialties, experience_years, certifications, phone, email, instagram, telegram, rating, total_reviews, is_verified, is_active)
WHERE NOT EXISTS (SELECT 1 FROM public.coaches WHERE name = v.name);