...
-- Update coaches table to add missing columns
...
ALTER TABLE public.coaches ADD COLUMN IF NOT EXISTS gym_id UUID REFERENCES public.gyms(id);
...
-- Update policies for coaches table
...
-- Update sample coaches data
UPDATE public.coaches SET 
  name = 'احمد محمدی',
  bio = 'مربی پرورش اندام با ۱۰ سال تجربه در زمینه بدنسازی و تناسب اندام',
  specialties = ARRAY['پرورش اندام', 'کاهش وزن', 'تناسب اندام'],
  experience_years = 10,
  certifications = ARRAY['مربی بدنسازی درجه یک', 'متخصص تغذیه ورزشی'],
  phone = '09121234567',
...
-- Insert additional sample coaches if table is empty
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_gyms_updated_at') THEN
